...
function stringContainsAny(str, arr) {
  for (var i = 0; i < arr.length; i++)
    if (stringContains(str, arr[i])) return true;
  return false;
}
...
function isValidIdentifier(str) {
  if (!str.toLowerCase().startsWith("aos://")) return false;
  ...
  if (split.length > 1 && !isValidPort(split[1])) return false;

  return true;
}

function ip2aos(ip, port, version) {
  if (!isIP(ip)) return false;

  var octets = ip.split(".");
  var identifier =
    parseInt(octets[3]) * 0x1000000 +
    parseInt(octets[2]) * 0x10000 +
    parseInt(octets[1]) * 0x100 +
    parseInt(octets[0]);
...
function getLauncherReleases(callback, tag) {
  ...
  $.get(
    httpsify("https://launcher.buildandshoot.com/update?json=true"),
    function (response) {
...
function getOpenspadesReleases(callback, tag) {
  ...
  $.get(
    "https://api.github.com/repos/yvt/openspades/releases/latest",
    function (response) {
...
function getPysnipReleases(callback, tag) {
  ...
  $.get(
    "https://api.github.com/repos/NateShoffner/PySnip/releases",
    function (response) {
      var linux = [];
...
function getPiqueserverReleases(callback, tag) {
  ...
  $.get(
    "https://api.github.com/repos/piqueserver/piqueserver/releases",
    function (response) {
      var linux = [];
...
function getVoxlapReleases(callback, tag) {
  ...
  $.get(
    httpsify("http://services.buildandshoot.com/game_mirrors"),
    function (response) {
      for (var i = 0; i < response.length; i++) {
